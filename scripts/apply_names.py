...
plan=json.load(open("/home/claude/pipeline/name_plan.json"))
DONE="/home/claude/pipeline/applied_ids.txt"
done=set()
...
def patch(p):
    ...
    for a in range(4):
        try:
            ...
            b=e.read().decode()[:160]
            if e.code in (429,502,503,504) and a<3: time.sleep(5*(a+1)); continue
