...
# Which data directory to import. Defaults to California; pass a different
# one to load another state, e.g.
#   AIRPLANE_DATA_DIR=data/arizona bash scripts/import_california.sh
DATA="${AIRPLANE_DATA_DIR:-data/california}"
[[ -d "$DATA" ]] || { echo "ERROR: no such data dir: $DATA" >&2; exit 2; }
...
already_loaded() {  # already_loaded <csv>
    local file="$1" result
    # TOP-UP files are the exception. A file named *topup* was built by
    # diffing against live data — its whole purpose is to add aircraft to a
    # museum that ALREADY has some. The "does this museum have aircraft?"
    # test is exactly wrong for them, and silently skipped the Palm Springs
    # top-up (43 aircraft, 21 waiting to be added). Let them through.
...
echo "═══════════════════════════════════════════════════════════════"
echo "  Import: $DATA ${DRY:+(DRY RUN — nothing will be written)}"
echo "  Host: $HOST"
echo "═══════════════════════════════════════════════════════════════"
