...
# Georgia import.
#
#   AIRPLANE_KEY=amt_... bash data/georgia/import_georgia.sh [--apply]
#
...
step museums data/georgia/ga_museums.csv true dry-run || { echo "STOP: museums file is bad"; exit 1; }
...
step museums data/georgia/ga_museums.csv false apply || { echo "STOP: museum apply failed"; exit 1; }

echo "== 3. dry-run all aircraft =="
fail=0
for f in data/georgia/*aircraft.csv; do step aircraft "$f" true dry-run || fail=1; done
if [ "$fail" != "0" ]; then
  echo
  echo "STOP: aircraft dry runs failed. Museums ARE now imported; fix the files"
  echo "and re-run only step 4 (do not re-apply ga_museums.csv)."
  exit 1
fi

echo "== 4. apply all aircraft =="
for f in data/georgia/*aircraft.csv; do step aircraft "$f" false apply || fail=1; done

echo
[ "$fail" = "0" ] && echo "ALL APPLIED - run verify_georgia.sh" || echo "APPLY FAILURES ABOVE"
exit $fail
