# Research brief — the Mid-Atlantic preserved-aircraft sweep (airplane.museum database)

You are researching preserved/displayed aircraft in New Jersey, Maryland, West Virginia, Delaware and the District of Columbia for a public database
that answers "where can I go and see this airframe". Today is September 2026.
New Jersey, Maryland and West Virginia are GREENFIELD. Delaware and DC each already hold a museum that must be TOPPED UP under its EXACT existing name, never re-created:
`Air Mobility Command Museum` (Dover DE, 2 rows recorded, ~35 real) and
`Smithsonian National Air and Space Museum` (Washington DC, 1 row recorded, ~60 real on the Mall).
Note the Steven F. Udvar-Hazy Center is a SEPARATE existing record in Virginia with 147 aircraft - do not touch it and do not record Virginia.
