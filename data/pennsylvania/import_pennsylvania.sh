...
# Pennsylvania import.
#
#   AIRPLANE_KEY=amt_... bash data/pennsylvania/import_pennsylvania.sh [--apply]
#
...
step museums data/pennsylvania/pa_museums.csv true dry-run || { echo "STOP: museums file is bad"; exit 1; }
...
step museums data/pennsylvania/pa_museums.csv false apply || { echo "STOP: museum apply failed"; exit 1; }

echo "== 3. dry-run all aircraft =="
fail=0
for f in data/pennsylvania/*_aircraft.csv; do step aircraft "$f" true dry-run || fail=1; done
if [ "$fail" != "0" ]; then
  echo
  echo "STOP: aircraft dry runs failed. Museums ARE now imported; fix the files"
  echo "and re-run only step 4 (do not re-apply pa_museums.csv)."
  exit 1
fi

echo "== 4. apply all aircraft =="
for f in data/pennsylvania/*_aircraft.csv; do step aircraft "$f" false apply || fail=1; done

echo
[ "$fail" = "0" ] && echo "ALL APPLIED - run verify_pennsylvania.sh" || echo "APPLY FAILURES ABOVE"
exit $fail
