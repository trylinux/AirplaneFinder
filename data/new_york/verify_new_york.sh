#!/bin/bash
# Verify the New York import against the live API: every site present, and each
...
while True:
    ...
    for m in j["results"]:
        if m.get("state_province")=="New York": live[m["name"]]=m["id"]
    if p>=j["pages"]: break
    p+=1

want={}
for r in csv.DictReader(open("data/new_york/ny_museums.csv")):
    want[r["name"]]=0
for f in sorted(glob.glob("data/new_york/*aircraft.csv")):
    rows=list(csv.DictReader(open(f)))
    name=rows[0]["museum_name"]
    want[name]=want.get(name,0)+len(rows)

bad=0
print(f"live New York sites: {len(live)}   expected: {len(want)}\n")
...
if extra: print("\n  unexpected New York sites:", ", ".join(sorted(extra))); bad=1
