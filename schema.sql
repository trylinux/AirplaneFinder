...
-- One write-up per designation, inherited by every airframe whose
-- normalized model+variant matches. Deliberately not manufacturer-scoped:
-- a Fuji-built UH-1H is still a UH-1H. Resolution is two-tier at render
-- time (exact variant, then base model) rather than an FK on aircraft, so
-- there is nothing to backfill and newly imported airframes inherit
-- immediately. See migrate_aircraft_types.sql and models.py.
-- ─────────────────────────────────────────────
